...
type FQDNDataClient interface {
	...
	StreamPolicyState(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[PolicyStateAck, PolicyState], error)
...
func (c *fQDNDataClient) StreamPolicyState(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[PolicyStateAck, PolicyState], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &FQDNData_ServiceDesc.Streams[0], FQDNData_StreamPolicyState_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[PolicyStateAck, PolicyState]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type FQDNData_StreamPolicyStateClient = grpc.BidiStreamingClient[PolicyStateAck, PolicyState]
...
type FQDNDataServer interface {
	...
	// In case of any client side error, cilium agent will cancel the stream and SDP will have to re-subscribe.
	// In case of any server side error, cilium agent will send an error response and SDP will have to re-subscribe.
	StreamPolicyState(grpc.BidiStreamingServer[PolicyStateAck, PolicyState]) error
	// UpdatesMappings is used by the Standalone DNS proxy to update the DNS mappings.
	// In case of any error, SDP will either retry the connection if the error is server side or will error out.
	// Note: In case of concurrent updates, since this is called in a callback(notifyDNSMsg) from the DNS server it follows the same semantics as
	// the inbuilt dns proxy in cilium.
...
func (UnimplementedFQDNDataServer) StreamPolicyState(grpc.BidiStreamingServer[PolicyStateAck, PolicyState]) error {
...
func _FQDNData_StreamPolicyState_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(FQDNDataServer).StreamPolicyState(&grpc.GenericServerStream[PolicyStateAck, PolicyState]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type FQDNData_StreamPolicyStateServer = grpc.BidiStreamingServer[PolicyStateAck, PolicyState]
