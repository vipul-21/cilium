...
func (d *Daemon) bootstrapFQDN(possibleEndpoints map[uint16]*endpoint.Endpoint, preCachePath string, ipcache fqdn.IPCache) (err error) {
	...
	// bootstrap the standalone dns proxy server
	d.bootstrapFQDNServer()
...
// FQDN Server is used to communicate with the Standalone DNS Proxy
func (d *Daemon) bootstrapFQDNServer() {
	fqdnserver := service.NewServer(&d.endpointManager, d.updateOnDNSMsg)
	proxy.GlobalStandaloneDNSProxy = fqdnserver

	go service.RunServer(option.Config.ToFqdnsServerPort, fqdnserver)
}
