...
func setupDNSProxyTestSuite(tb testing.TB) *DNSProxyTestSuite {
	...
	proxy, err := StartDNSProxy(dnsProxyConfig, // any address, any port, enable ipv4, enable ipv6, enable compression, max 1000 restore IPs
		...
		func(ip netip.Addr) (*endpoint.Endpoint, bool, error) {
			...
			return endpoint.NewTestEndpointWithState(s, s, testipcache.NewMockIPCache(), &endpoint.FakeEndpointProxy{}, testidentity.NewMockIdentityAllocator(nil), uint16(epID1), endpoint.StateReady), false, nil
		},
		// LookupSecIDByIP
...
func TestCheckNoRules(t *testing.T) {
	...
	// StandaloneDNSProxy
	dnsPoliciesEmpty := &standalonednsproxy.DNSPolicies{
		EgressL7DnsPolicy: []*standalonednsproxy.DNSPolicy{
			{
				SourceIdentity: epIdentity,
				DnsPattern:     []string{},
...
func TestFullPathDependence(t *testing.T) {
	...
	err = s.proxy.UpdateAllowedIdentities(&standalonednsproxy.DNSPolicies{
		EgressL7DnsPolicy: []*standalonednsproxy.DNSPolicy{
			{
				SourceIdentity: epIdentity,
	...
	// Restore rules
	ep1 := endpoint.NewTestEndpointWithState(s, s, testipcache.NewMockIPCache(), &endpoint.FakeEndpointProxy{}, testidentity.NewMockIdentityAllocator(nil), uint16(epID1), endpoint.StateReady)
	...
	ep3 := endpoint.NewTestEndpointWithState(s, s, testipcache.NewMockIPCache(), &endpoint.FakeEndpointProxy{}, testidentity.NewMockIdentityAllocator(nil), uint16(epID3), endpoint.StateReady)
